  ...
  return {
    ...
    commands: {
      ping: {
        ...
        action: msg => {
          const shard = !msg.channel.type ? msg.channel.guild.shard : client.shards.get(0)
          const clientLatency = Date.now() - msg.timestamp
          return {
            embed: {
              title: 'Bot Status',
              description: `Pong! Client: **${clientLatency}ms** API: **${shard.latency}ms** | Servers: **${client.guilds.filter(g => g.shard === shard.id).length}**`,
              ...
              // ARG COMPILER
                (this._commands[item].args
                  ? this._commands[item].args.reduce((a, e) =>
                    (a + (e.mand ? `<${e.name}>` : `(${e.name})`) + (e.delim || ' ')).slice(0, -1 * e.delim ? e.delim.length : -1), ' '
                  )
                  // DESCRIPTION
      ...
      updates: {
        ...
        args: [{ name: 'version' }],
        action: (msg, [ver]) => {
          const updates = fs.readFileSync('data/updates.txt', 'utf8')
          const version = ver && (ver.match(/\./g) || []).length >= 2 ? ver : require(path.join(process.cwd(), './package.json')).version
      ...
      replacespace: {
        ...
        action: (msg, [replace, text]) => text.replace(/\s/g, replace)
      },
      math: {
